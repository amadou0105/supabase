...
ErrorDisplay renders a styled error card with a warning header, a monospace error message block, an optional children slot for inline troubleshooting content, and an optional "Contact support" footer link.
...
<ErrorDisplay
  ...
  supportUrl="/support/new?projectRef=my-project"
/>
```
...
### Without support link

Omit `supportUrl` to hide the footer entirely.

<ComponentPreview name="error-display-no-support" wide />
...
| `supportUrl`     | `string?`   | `undefined`         | URL for the "Contact support" footer link. Omit to hide the footer.    |
