...
- Prefer `browserTitle.section` and `browserTitle.surface` when composing tab titles
- Use `browserTitle.entity` for the most specific resource (table/function/query) when available
- `ProjectLayout.title` remains supported as legacy/deprecated compatibility
