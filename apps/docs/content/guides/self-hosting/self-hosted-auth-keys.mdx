...
Self-hosted Supabase can be configured to use the [new API keys](/docs/guides/api/api-keys) alongside the legacy API keys (`ANON_KEY` and `SERVICE_ROLE_KEY` HS256-signed JWTs).
...
- If you are upgrading an exising self-hosted Supabase environment, make sure to check the [changelog](https://github.com/supabase/supabase/blob/master/docker/CHANGELOG.md) and add/update the following files:
...
| `JWT_JWKS` | JWKS (JSON) | **New:** Contains the new asymmetric keypair and the legacy symmetric key. Used by PostgREST, Realtime, and Storage to verify tokens. |
...
1. Client sends `apikey: sb_publishable_xxx` (or legacy `apikey: eyJ...`).
2. API gateway checks the key and identifies the consumer (`anon` or `service_role`).
3. API gateway inspects the `Authorization` header. Since it is either absent or starts with `Bearer sb_` (an opaque key, not a session JWT), the plugin replaces it:
   - **New `sb_` key:** `Authorization` header is set to the internal pre-signed ES256 JWT that corresponds to the role.
   - **Legacy JWT key:** `Authorization` header is set to the legacy HS256 JWT (the `apikey` value is copied as-is).
...
1. Client sends `Authorization: Bearer eyJ...` (a JWT session token from Auth) alongside `apikey: sb_publishable_xxx` (or legacy `apikey: eyJ...`).
2. API gateway checks the API key and identifies the consumer.
3. API gateway inspects the `Authorization` header. Since it exists and does **not** start with `Bearer sb_` (it's a real JWT, not an `sb_` API key), the plugin **passes it through unchanged**. This works the same way regardless of whether the `apikey` is a new `sb_` key or a legacy JWT - the gateway only looks at the `Authorization` header to decide whether a user session is present.
