...
This guides uses [Vite with Vue 3 Template](https://vitejs.dev/guide/#scaffolding-your-first-vite-project) to initialize
...
Set up an `src/components/Auth.vue` component to manage logins and sign ups using Magic Links, so users can sign in with their email without using passwords.
...
Now that you have all the components in place, update `App.vue`:
...
And then add the widget to the Account page. The `Account.vue` component [shown earlier](#account-page) already includes the `Avatar` component when using the full example code.
